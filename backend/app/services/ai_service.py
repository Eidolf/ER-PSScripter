...
class AIService:
    ...
    def _get_config(self, db: Session) -> Dict[str, str]:
    ...
    def _construct_system_prompt(self, context_snippets: List[Snippet]) -> str:
        ...
        prompt += (
            "If you use an existing snippet, assume it is available in the runspace (do not redefine it unless necessary, or wrap it in a 'Function' block if asked).\n"
            "Provide ONLY the PowerShell code in your response, wrapped in a markdown code block (```powershell ... ```).\n"
