...
export interface TerminalRef {
    ...
    runScriptFile: (filename: string) => void;
}
    ...
    useImperativeHandle(ref, () => ({
        ...
        uploadFile: (filename: string, content: string) => {
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                ...
                const contentB64 = btoa(unescape(encodeURIComponent(content))); // Handle UTF-8
                wsRef.current.send(`file:${filename}:${contentB64}`);
            } else {
                console.warn("Terminal WS not ready for upload");
            }
        },
        runScriptFile: (filename: string) => {
            sendTextToTerminal(`& ./${filename}\r`);
        }
    }));
